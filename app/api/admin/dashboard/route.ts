...
type DashboardVenda = Pick<VendaRow, 'created_at' | 'status' | 'tipo' | 'total_final' | 'total_original'>;
...
async function loadVendas(supabase: ReturnType<typeof getSupabaseAdmin>) {
  const rows: DashboardVenda[] = [];
  for (let from = 0; from < MAX_ROWS; from += PAGE_SIZE) {
    const { data, error } = await supabase.from('vendas').select('tipo, status, total_original, total_final, created_at').not('cliente_auth_user_id', 'is', null).is('deleted_at', null).range(from, from + PAGE_SIZE - 1);
    if (error) throw new Error(`Erro ao carregar vendas: ${error.message}`);
    if (!data?.length) break;
    rows.push(...(data as DashboardVenda[]));
    if (data.length < PAGE_SIZE) break;
  }
  return rows;
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const openCarts = vendas.filter(isOpenCart);
    const completedSales = vendas.filter(isCompletedSale);
    const now = Date.now();
    const averageWaitMs = openOrders.length ? openOrders.reduce((sum, order) => sum + Math.max(0, now - new Date(order.created_at).getTime()), 0) / openOrders.length : 0;
    ...
    const sumSince = (key: string) => completedSales.filter((venda) => venda.created_at >= startIso(key)).reduce((sum, venda) => sum + getVendaValue(venda), 0);
    ...
    return NextResponse.json({
      ...
      orders: { averageWaitMs, completed: completedOrders.length, open: openOrders.length },
